...
import { communitiesQuery, projectsQuery } from "@/lib/atlas-queries";
  ...
  return (
    <PageShell>
      <PageHeader
        ...
        title={`${communities.length} communities. One shared ledger.`}
      ...
      <section className="px-6 py-12">
        <div className="mx-auto grid max-w-7xl gap-6 md:grid-cols-2 lg:grid-cols-3">
          {communities.map((c) => {
            const projCount = projects.filter((p) => p.community_id === c.id).length;
            return (
              <article
                key={c.id}
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="mt-1 font-medium tabular-nums">{projCount}</p>
                  </div>
                </div>
                {c.needs && (
                  <div className="mt-5">
